	...
	_sendRequest: function( url, method, params, successcallback, failedcallback ){
		...
		url, 
		{
		   ...
		   onFailure		: function(transport) 
		   {
				...
				{
					this._showErrorMessageBox(ORYX.I18N.Oryx.title, ORYX.I18N.VisioBPMNImport.connectionError);
...
// -------------------------------------------- Import Functions ------------------------
			...
			buttons:[
				{
					...
					handler:function(){
						...
						var loadMask = new Ext.LoadMask(Ext.getBody(), {msg:ORYX.I18N.VisioBPMNImport.progress});
						loadMask.show();
	...
	{
		var url = ORYX.CONFIG.VISIOBPMNIMPORT;
		
		Ext.Ajax.request({
			url: url,
			method: 'POST',
			success: function(request){this.facade.importJSON(request.responseText); loadMask.hide()}.bind(this),
			failure: function() {loadMask.hide(); Ext.Msg.alert(ORYX.I18N.VisioBPMNImport.connectionError);},
			params: {data: vdx,
					 action: "Import"}
		});
