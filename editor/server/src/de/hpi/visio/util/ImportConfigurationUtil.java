...
/**
 * Provides all properties for visio import: Uses the VisioImportConfiguration.xml  
 */
public class ImportConfigurationUtil {
		...
		if (in != null) {
			try {
				...
				try {
					...
					throw new RuntimeException("Not able to create the Visio Stencil Set to BPMN mapping properties.", e);
				}
			}
		}			
	}
	...
	/**
	 * Additional mapping configuration for shape merging (shape is only a marker for
	 * another shape (e.g., +-marker element to make a task to a collapsed subprocess)
	 * and to set additional properties (e.g., istransaction or is MI-task)
	 */
	...
	/**
	 * Additional configuration for the stencil set at oryx:
	 * Minimal sizes or fixed sizes for some elements of a stencilset can be defined
	 */
	...
	/**
	 * Heuristic values for given keys. Values can be changed in the configuration xml
	 * and are defined for free-text-interpretation, default page sizes, size-unit-exchange-factor
	 * from visio to oryx and for edge-assignment.
	 */
	 ...
	 * interpretation (e.g., What size must a task have to be interpreted as subprocess)
	 */
